...
/**
 * Defines full path to current DSF installation.
 *
 * @var string
 */
define ( 'DSF_ROOT', realpath ( __DIR__ ) );
...
class DSF {
	 ...
	 *
	 *
	 * {@link DSF::init() Initializes} this class and routes the request to the
	 * controller/method.
	 ...
	 *
	 * DSF supports named mappings for arguments in query.<br>
	 ...
	 * <ol>
	 * <li>loads DSF_ROOT/engine/lib/functions.php</li>
	 * <li>Registers {@link DSF::classLoader()} as class autoloader.</li>
	 * <li>Consequently loads DSF_ROOT/app/config/dsf.php and
	 * APP_ROOT/app/config/app.php (values from app.php will override those from
	 * dsf.php)</li>
	 */
	private static function init() {
		require_once DSF_ROOT . '/engine/lib/functions.php';
		spl_autoload_register ( 'DSF::classLoader' );
		CFG::load ( DSF_ROOT . '/app/config/dsf.php' );
	 ...
	 * (suppose DSF is installed in /dsf and your application is /proj and the
	 * class you wanted is \\app\\my\\Class)
	 * <ol>
	 * <li>try to load /proj/app/my/Class.php</li>
	 * <li>try to load /dsf/app/my/Class.php</li>
	 * </ol>
	 * In this way you can replace standard DSF classes.<br/>
	 * Just create <code>/proj/engine/very/same/path/as/in/dsf/ThatClass.php</code><br
	 * />
	 * and ofcourse define the class:<br />
	 * <code>
	 * &lt;?<br>
	 * namespace engine\\very\\same\\path\\as\\in\\dsf;<br>
	 * class ThatClass{}<br>
	 * ?&gt;
	 * </code><br>
	...
	/**
	 * Loads a PHP file either from your project or from DSF folders.
	 * <p>
	 * Tries to sanitize the path (<code>$what</code>), ensures ".php" extension
	 * (so you can omit it) then looks first into your project folder then in DSF
